...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Runtime Markdown renderer for Gramps gramplets.

Renders the md2gramplet markdown dialect directly into a Gtk.TextBuffer,
with support for images, inline formatting, and gramps: URI scheme links.

Public API:
    render_md(text, buf, links, dbstate=None, uistate=None, vars=None)
    load_md(plugin_dir, basename)  -- locale-aware .md file loader
    setup_tags(buf)                -- create standard text tags on buf
"""
...
def _load_pixbuf(path, width=None):
    """Load and optionally scale a pixbuf.

    Handles URI schemes:
        gramps:icon:name:size  -- GTK theme icon via Gtk.IconTheme
        gramps:image:filename  -- file relative to IMAGE_DIR
        plain/path             -- file relative to IMAGE_DIR (legacy)
    """
...
def _ins_image(buf, path, width=None, align="left"):
...
def _ins_link(buf, links, text, url, tag="link"):
...
def _render_inline(buf, text):
    """Parse **bold** and *italic* in text and insert into buf with tags."""
    pos = 0
    for m in _INLINE_RE.finditer(text):
        if m.start() > pos:
            buf.insert(buf.get_end_iter(), text[pos : m.start()])
        inner = m.group(2) if m.group(2) is not None else m.group(3)
        tag = "bold" if m.group(2) is not None else "italic"
        start_offset = buf.get_char_count()
...
def url_to_value(url, vars=None):
    """Resolve a markdown URL spec to a string at runtime.

    Handles wiki:, wiki_manual:, {VARNAME} substitution, and literal URLs.
    """
...
def _handle_gramps_link(url, dbstate, uistate):
...
def setup_tags(buf):
    """Create all standard tags on buf (idempotent — skips existing tags)."""
    ...
    _ensure("h1", weight=Pango.Weight.BOLD, scale=1.728)  # Pango XX_LARGE
    _ensure("h2", weight=Pango.Weight.BOLD, scale=1.440)  # Pango X_LARGE
    _ensure("h3", weight=Pango.Weight.BOLD, scale=1.200)  # Pango LARGE
...
def load_md(plugin_dir, basename):
    ...
    """
    lang = glocale.lang  # e.g., "fr_FR.UTF-8" or "fr_FR"
    lang_full = lang.split(".")[0]  # e.g., "fr_FR"
    lang_short = lang_full.split("_")[0]  # e.g., "fr"
...
def render_md(text, buf, links, dbstate=None, uistate=None, vars=None):
    """Parse and render a markdown string into buf.

    Tags must already exist on buf — call setup_tags(buf) first, or use a
    gramplet whose build_gui() already creates them.

    Args:
        text:    Markdown source (already in the desired locale — no _() applied).
        buf:     Gtk.TextBuffer to render into (cleared before rendering).
        links:   List to which (start_offset, end_offset, url) tuples are
                 appended for each link.
        dbstate: Gramps DbState — required for gramps:edit/nav links.
        uistate: Gramps UiState — required for gramps:edit/nav/view links.
        vars:    Dict of variable name -> value for {VARNAME} URL substitution.
    """
